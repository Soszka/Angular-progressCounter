...
export class TrainingDataService {
  private baseUrl =
    'https://progress-counter-8443f-default-rtdb.europe-west1.firebasedatabase.app/database';
  ...
  getTrainings(): Observable<Training[]> {
    ...
    return this.http
      .get<{ [key: string]: Exercise[] }>(`${this.baseUrl}/users/${uid}.json`)
      .pipe(
        map((response) => {
  ...
  deleteTraining(trainingName: string): Observable<void> {
    ...
    return this.http.delete<void>(
      `${this.baseUrl}/users/${uid}/${trainingName}.json`
    );
  }
  ...
  addExercise(trainingName: string, exerciseName: string): Observable<void> {
    ...
    return this.http
      .get<Exercise[]>(`${this.baseUrl}/users/${uid}/${trainingName}.json`)
      .pipe(
        ...
        switchMap((existingExercises) => {
          const updatedExercises = existingExercises.map((exercise) => {
            if (exercise.name === exerciseName) {
              return {
                ...exercise,
                dailyData: exercise.dailyData.filter(
                  (data) => data.date !== date
                ),
              };
            }
